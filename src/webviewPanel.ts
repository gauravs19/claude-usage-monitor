...
function renderCardsForRange() {
  ...
  document.getElementById('cards').innerHTML = buildCards(ranged, rIn, rOut, rCost, cacheHit, label, today, null, null, _allSessions, null, null, 0, true);
}
...
function buildCards(ranged, rIn, rOut, rCost, cacheHit, label, today, latest, liveMetrics, allSessions, fiveHourPct, sevenDayPct, ctxPct, ctxStale) {
  ctxPct = ctxPct ?? 0;
  const ctxColor = ctxStale ? 'var(--muted)' : ctxPct >= 80 ? 'var(--red)' : ctxPct >= 60 ? 'var(--yellow)' : 'var(--green)';
  const ctxSub = ctxStale ? 'no active session' : (latest ? fmtK(latest.tokensPerTurn??0)+'/turn' : '—');
  ...
  return [
    ...
    cardWithBar('Context', ctxStale ? '—' : Math.round(ctxPct)+'%', ctxSub, ctxPct, ctxColor,
...
window.addEventListener('message', e => {
  ...
  const hasLive = liveMetrics != null;

  // Today-only data for time-saved estimates (range totals produce absurd values)
  ...
  // Context: only use live data or fresh session (last 2h); stale JSONL ctx shown as grey
  const latestTs = latest?.lastTs ?? '';
  const sessionFresh = latestTs && (Date.now() - new Date(latestTs).getTime()) < 2 * 3600000;
  const ctxPct = hasLive ? (liveMetrics.contextPct ?? 0) : (sessionFresh ? (latest?.contextPct ?? 0) : 0);
  const ctxStale = !hasLive && !sessionFresh;

  const fiveHourPct = liveMetrics?.fiveHourPct ?? null;
  const sevenDayPct = liveMetrics?.sevenDayPct ?? null;

  document.getElementById('cards').innerHTML = buildCards(ranged, rangedIn, rangedOut, rangedCost, cacheHitPct, rangeLabel, today, latest, liveMetrics, sessions, fiveHourPct, sevenDayPct, ctxPct, ctxStale);
  ...
  // Efficiency uses today's token totals — range totals produce absurd time-saved values
...
function renderEfficiency(eff, todayIn, todayOut) {
  ...
  // Only show alerts when hooks are active (tool call data is meaningful)
  const alertsHtml = noActivity ? '' : [
